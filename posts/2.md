...
In terms of preventing data in durable storage from being succeptible to power failure during a transaction, we're not quite there yet. We have, however, created a tool that gives us a powerful play-by-play logging to identify *when* the bug has occurred. As it turns out, we also have the information we need to bring persisted data back to a stable state when we've identified an issue. We'll work on implementing a recovery routine in the next post!
